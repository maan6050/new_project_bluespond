<div class="space-y-6">
    ...
    <div class="rounded-xl border border-teal-100 bg-teal-50/60 p-4">
        <p class="text-sm font-semibold text-slate-900">{{ __('Your public booking URL') }}</p>
        <p class="mt-1 break-all font-mono text-sm text-teal-700">
            @if ($this->publicBookingUrl())
                {{ $this->publicBookingUrl() }}
            @else
                <span class="italic text-teal-600/70">{{ __('Will be generated after step 1') }}</span>
            @endif
        </p>
    </div>

    <label class="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-white p-4 transition-colors hover:border-teal-300 hover:bg-teal-50/40">
        <input type="checkbox" wire:model="isPublished"
               class="mt-1 rounded border-slate-300 text-teal-600 transition-colors focus:ring-2 focus:ring-teal-500/20">
        <div>
